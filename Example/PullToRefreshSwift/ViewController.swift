...
//        インジゲーターの色
        options.indicatorColor = .red
//        引っ張って伸びてきた部分の背景色。ここを設定するとインジゲーターが塗りつぶされ、表示されなくなる
        options.fixedSectionHeader = true
        
        self.tableView.addPullRefresh(options: options) { [weak self] in
            // some code
            sleep(1)
            self?.texts.shuffle()
